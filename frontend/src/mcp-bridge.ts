...
//    - batch_create: ONE call, ONE layer group containing all shapes. Efficient.
...
const TOOLS = [
	...
	{ name: "batch_create", description: "Create many simple shapes in ONE call, producing ONE layer group containing all shapes. Each item has a 'type' field ('rectangle' | 'ellipse' | 'line' | 'text') plus type-specific fields (x, y, width, height, radius_x, radius_y, x1, y1, x2, y2, fill_color, stroke_color, stroke_width, text, font_size, corner_radius). All coordinates are in document space. Efficient for any number of shapes — all become a single layer group.", inputSchema: { type: "object", properties: { shapes: { type: "array", items: { type: "object" }, description: "Array of shape objects to create." } }, required: ["shapes"] } },
